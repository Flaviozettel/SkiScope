...
Die Wetterdaten stammen von [Open-Meteo](https://open-meteo.com/), einer frei zugänglichen Wetter-API für Prognose- und historische Wetterdaten. Im SkiScope-Backend werden die abgerufenen Prognosedaten zwischengespeichert, um unnötige API-Anfragen zu vermeiden. Für jedes Skigebiet wird eine neue Prognose nur dann von Open-Meteo geladen, wenn der vorhandene Eintrag im Datenbank-Cache älter als drei Stunden ist. Details dazu sind auf der [Server-Seite]({{ '/architektur_gdi.html#server' | relative_url }}) beschrieben.
